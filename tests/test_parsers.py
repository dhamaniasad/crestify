...
from crestify.services.parsers import (
    PinboardParser,
    ReadabilityParser,
    InstapaperParser,
    PocketParser,
...
class PinboardTestCase(unittest.TestCase):
    def setUp(self):
        ...
        with open("Pinboard.json") as json_file:
            create_file = open(
                os.path.join(
                    app.config["CRESTIFY_UPLOAD_DIRECTORY"], "test_pinboard.json"
                ),
                "w+",
            )
        ...
        for bmark in self.query:
            self.assertEqual(
                datetime.datetime.strptime(
                    self.bookmarks[bmark.main_url]["time"], "%Y-%m-%dT%H:%M:%SZ"
                ),
                bmark.added_on,
            )
    ...
    def test_tags(self):
        for bmark in self.query:
            tags = [
                _f
                for _f in self.bookmarks[bmark.main_url]["tags"].split(" ")
                + ["Imported"]
                if _f
            ]
    ...
    def test_description(self):
        for bmark in self.query:
            self.assertEqual(
                (
                    self.bookmarks[bmark.main_url]["extended"]
                    if self.bookmarks[bmark.main_url]["extended"] != ""
                    else None
                ),
                bmark.description,
            )
    ...
    def tearDown(self):
        ...
        db.session.commit()
        os.remove(
            os.path.join(app.config["CRESTIFY_UPLOAD_DIRECTORY"], "test_pinboard.json")
        )
...
class ReadabilityTestCase(unittest.TestCase):
    def setUp(self):
        ...
        with open("Readability.json") as json_file:
            create_file = open(
                os.path.join(
                    app.config["CRESTIFY_UPLOAD_DIRECTORY"], "test_readability.json"
                ),
                "w+",
            )
            self.data = json_file.read()
    ...
    def test_urls(self):
        for bmark in self.query:
            self.assertEqual(
                self.bookmarks[bmark.main_url]["article__url"], bmark.main_url
            )

    def test_titles(self):
        for bmark in self.query:
            self.assertEqual(
                self.bookmarks[bmark.main_url]["article__title"], bmark.title
            )
    ...
    def test_timestamps(self):
        for bmark in self.query:
            ...
            self.assertEqual(
                datetime.datetime.strptime(
                    self.bookmarks[bmark.main_url]["date_added"], "%Y-%m-%dT%H:%M:%S"
                ),
                bmark.added_on,
            )
        ...
        for bmark in self.query:
            self.assertEqual(
                self.html_parser.unescape(
                    (self.bookmarks[bmark.main_url]["article__excerpt"])
                    if self.bookmarks[bmark.main_url]["article__excerpt"] != ""
                    else None
                ),
                bmark.description,
            )
...
class InstapaperTestCase(unittest.TestCase):
    def setUp(self):
        ...
        with open("Instapaper.html") as json_file:
            create_file = open(
                os.path.join(
                    app.config["CRESTIFY_UPLOAD_DIRECTORY"], "test_instapaper.html"
                ),
                "w+",
            )
            ...
            for tag in self.html_data.find_all("h1"):
                ...
                for link in links:
                    ...
                    self.bookmarks[url] = {"href": url, "title": title, "tags": tags}
            create_file.write(self.data)
            self.file_path = create_file.name
            create_file.close()
...
class PocketTestCase(unittest.TestCase):
    def setUp(self):
        ...
        with open("Pocket.html") as json_file:
            create_file = open(
                os.path.join(
                    app.config["CRESTIFY_UPLOAD_DIRECTORY"], "test_pocket.html"
                ),
                "w+",
            )
            self.data = json_file.read()
    ...
    def tearDown(self):
        ...
        db.session.commit()
        os.remove(
            os.path.join(app.config["CRESTIFY_UPLOAD_DIRECTORY"], "test_pocket.html")
        )
...
class NetscapeTestCase(unittest.TestCase):
    def setUp(self):
        ...
        with open("Netscape.html") as json_file:
            create_file = open(
                os.path.join(
                    app.config["CRESTIFY_UPLOAD_DIRECTORY"], "test_netscape.html"
                ),
                "w+",
            )
            self.data = json_file.read()
    ...
    def tearDown(self):
        ...
        db.session.commit()
        os.remove(
            os.path.join(app.config["CRESTIFY_UPLOAD_DIRECTORY"], "test_netscape.html")
        )
