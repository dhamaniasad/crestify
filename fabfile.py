...
def install_postgres():
  ...
  sudo apt-key add -"
    ...
    sudo("apt-get update")
    sudo(
        "apt-get -y install postgresql-9.4 postgresql-server-dev-9.4 postgresql-contrib-9.4"
    )
    ...
    with settings(sudo_user="crestify", shell="/bin/bash -c"):
        with cd("/home/crestify"):
            with cd("crestify"):
                ...
                sudo(
                    "echo 'CRESTIFY_CONFIG_FILE=\"/home/crestify/override.py\"' > .env"
                )
...
def setup_supervisor():
    ...
    for n, i in enumerate(content):
        ...
        if i.startswith("stopsignal="):
            content[
                n
            ] = "stopsignal=TERM"  # Both Gunicorn and Celery use SIGTERM for graceful shutdown
    content = StringIO("\n".join(content))
