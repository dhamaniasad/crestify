...
class Parser:
    def __init__(self, file_name, user_id):
        ...
        self.check_duplicates_query = Bookmark.query.filter(
            Bookmark.user == self.user, Bookmark.deleted == False
        ).all()
            ...
            if not self.valid_url.match(url):
                ...
                if url in self.urls:
                    ...
                    self.urls[url] = {
                        "title": link.text,
                        "tags": tags,
                        "added_on": datetime.datetime.utcfromtimestamp(added_timestamp),
                    }
    ...
    def add_to_database(self):
        len_urls = len(list(self.urls.items()))
        if len_urls > 400000:
            print("Sorry, cannot import, way too many bookmarks.")
            return
        counter = 0
        for url, metadata in list(self.urls.items()):
            counter += 1
            if counter != len_urls:
                if not url in self.check_duplicates:
                    self.transfer(
                        title=self.html_parser.unescape(metadata["title"]),
    ...
    def transfer(self, title, url, date, user, tags, commit=False):
...
class PinboardParser:
    def __init__(self, file_name, user_id):
        ...
        self.urls = (
            dict()
        )  # Keeps track of all the urls in the import file, used when adding to db
        self.tags_dict = dict()  # Store tag objects for imported bookmarks
        ...
        self.valid_url = re.compile(
            ...
            r"(?:/?|[/?]\S+)$",
            re.IGNORECASE,
        )  # We only want valid URLs in the database
    ...
    def process(self):
        ...
        for individ_tag in tags_query:
            tags_dictionary[
                individ_tag.text
            ] = individ_tag  # Store all existing tags in dict
        created_tags = {}  # Store all tags that had to be created
        for link in self.data:
            ...
            if not self.valid_url.match(url):  # If the url is not valid, burn it
                ...
                try:
                    ...
                    tags = []
                tags = [_f for _f in tags if _f]  # Remove empty strings from tag list
                tags.append(
                    "Imported"
                )  # This is used as a way of keeping track of Imported bookmarks in the db
                try:
                    # Pinboard stores timestamps in ISO format in UTC
                    added_timestamp = datetime.datetime.strptime(
                ...
                for tag in tags:
                    self.tags_set.add(
                        tag
                    )  # Add all tags from current bookmark to the master tag set. No dupes.
        for tag in self.tags_set:
            if (
                tag in self.tags_dict
            ):  # This tag has already been processed, and is in dict
                pass
                ...
                self.tags_dict[tag] = {
                    "obj": tags_dictionary[tag]
                }  # Add the tag to the main tags object dictionary
        ...
        for (
            ...
            q
        ):  # Here we check if this was one of the tags that had to be created, and add to main tags dict
            if tag.text in created_tags:
                self.tags_dict[tag.text] = {"obj": tag}
    ...
    def transfer(self, title, url, date, user, tags, description=None, commit=False):
...
class PocketParser:
    ...
    def process(self):
        ...
        for link in self.soup.find_all("a"):
            ...
            self.urls[url] = {"title": title, "tags": tags, "added_on": dt}
        for tag in self.tags_set:
            if tag in self.tags_dict:  # Tag has already been processed
                pass
            elif tag in created_tags:
            ...
            try:
                ...
                if type(doc) == list:
                    if "extended" in doc[0]:
                        ...
                        x = PinboardParser(
                            os.path.join(
                                app.config["CRESTIFY_UPLOAD_DIRECTORY"], filename
                            ),
                            user,
                        )
                        x.process()
                ...
                doc = data.splitlines()
                if (
                    doc[0] == "<!DOCTYPE NETSCAPE-Bookmark-file-1>"
                    and doc[5] == "<TITLE>Bookmarks</TITLE>"
                ):
                    """
