...
def require_auth(view_function):
    ...
    def decorated_function(*args, **kwargs):
        ...
        query = User.query.filter_by(
            email=urllib.parse.unquote(args["email"]),
            api_key=urllib.parse.unquote(args["apikey"]),
        ).first()
...
class CheckURL(Resource):
    ...
    def post(self, userid, args):
        if args["url"] is not None:
            parsed_url = urllib.parse.urlparse(urllib.parse.unquote(args["url"]))
            ...
            if query:
                return {
                    ...
                    "added": arrow.get(query.added_on).humanize(),
                    "timestamp": arrow.get(query.added_on).strftime(
                        "%Y-%m-%dT%H:%M:%SZ"
                    ),
        ...
        if args["url"] is not None:
            ...
            if query:
                ...
                user_tags = (
                    ...
                    .join(tags, Tag.id == tags.c.tag_id)
                    .join(Bookmark, Bookmark.id == tags.c.bookmark_id)
                    .filter(Bookmark.user == userid)
                    .all()
                )
...
class AddURL(Resource):
    ...
    def post(self, userid, args):
        if args["addedon"]:
            added = (
                args["addedon"] / 1000
            )  # Convert timestamp from milliseconds since epoch to seconds(Chrome sends millisecs)
            new_bookmark = bookmark.new(
                title=urllib.parse.unquote(args["title"]),
...
class EditBookmark(Resource):
    ...
    def post(self, userid, args):
        if args["id"]:
            ...
            tags = urllib.parse.unquote(args["tags"])
            tags = tags.split(",")
...
class AddTabs(Resource):
    ...
    def post(self, userid, args):
        if args["tabs"]:
            ...
            uuid = shortuuid.uuid()
            r = redis.set(uuid, urllib.parse.unquote(args["tabs"]))
