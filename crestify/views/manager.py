    ...
    if search_form.query.data:
        ...
        if search_form.parameter.data == "basic":
            ...
            if query.count() == user_count:
                query = query.filter(
                    or_(
                        Bookmark.title.contains(search_form.query.data),
            ...
            query = search(
                query, search_form.query.data, vector=Bookmark.fulltext_vector
            )
            user_count = Bookmark.query.filter_by(
                user=current_user.id, deleted=False
            ).count()
...
def bookmark_edit(bookmark_id=None):
    if bookmark_id:
        bookmark_id = hashids.decode(str(bookmark_id))[
            0
        ]  # Apparently, hashids.decode doesn't accept unicode input
        ...
        if form.validate_on_submit():
            ...
            return redirect(url_for("bookmark_list"))
        return render_template(
            "manager/bookmark_edit.html", form=form, bookmark_obj=bookmarkobj
        )
...
def bookmark_reader(bookmark_id=None):
    ...
    if bookmarkobj.readability_html is None:
        ...
        return render_template(
            "manager/bookmark_readable.html",
...
def bookmark_context(bookmark_id):
    bookmark_id = hashids.decode(str(bookmark_id))[0]
    query = (
        db.session.query(Bookmark.id)
        .filter_by(user=current_user.id, deleted=False)
        .order_by(Bookmark.added_on.desc())
        .all()
    )
...
def bookmark_delete():
    ...
    if deleteform.validate_on_submit():
        bookmark.delete(id=deleteform.id.data, user_id=current_user.id)
    return redirect(url_for("bookmark_list"))
    ...
    if len(request.form["value"]) > 255:
        return (
            jsonify(
                {
                    "status": "error",
                    "msg": "Title cannot be more than 255 characters long!",
                }
            ),
            400,
        )
    else:
        edit_tab = tab.edit_title(
            request.form["pk"], current_user.id, request.form["value"]
        )
        if edit_tab:
            return jsonify(""), 200
        else:
